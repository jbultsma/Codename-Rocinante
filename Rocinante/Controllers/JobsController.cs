...
namespace Rocinante.Controllers
{
    public class JobsController : Controller
    {
            ...
            // GET: Jobs/Details/5
            public async Task<IActionResult> DetailsJob(string id)
            {
                var loggedInUser = LoggedInUser();
                if (id == null)
                {
                ...
                var job = await _context.Job.Include(y => y.activities).FirstOrDefaultAsync(m => m.JobId == id && m.UserId == loggedInUser);
                if (job == null)
                {
                    return NotFound();
                }
